...
task <- conf$job$tasks$estimation_step2
args <- c(
  "--input", 
  sprintf("%s/CLV/estimation_tmp", tz.basedir),
  
  "--output", 
  sprintf("%s/CLV/estimation", tz.basedir),
  ...
  "--jri-name",
  "valuepotion.analytics.ltv.estimation-step2",
...
cat(print.timestamp(), "* Running ltv.estimation-step2.\n")
...
mr.run(
  ...
  files = list.files(path = file.path(getwd(), "mr", "estimation-step2"), pattern = "\\.R$", full.names = TRUE, recursive = TRUE),
  class = task$main,
  args = args,
