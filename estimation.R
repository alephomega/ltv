...
task <- conf$job$tasks$estimation
args <- c(
  "--input", 
  ...
  "--jri-name",
  "valuepotion.analytics.ltv.estimation",
...
mr.run(
  ...
  files = list.files(path = file.path(getwd(), "mr", "estimation"), pattern = "\\.R$", full.names = TRUE, recursive = TRUE),
  class = task$main,
  args = args,
